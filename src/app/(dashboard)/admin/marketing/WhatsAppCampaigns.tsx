...
export function WhatsAppCampaigns({ hotelBranding }: WhatsAppCampaignsProps) {
    ...
    // Core Campaign State
    const [audienceType, setAudienceType] = useState<'hotel_guests' | 'restaurant_customers'>('hotel_guests');
    ...
    const fetchTemplates = async () => {
        ...
        if (result.success) {
            // Only show APPROVED templates for actual campaigns
            setTemplates(result.data.filter((t: any) => t.status === 'APPROVED'));
        }
    ...
    const handleTemplateSelect = (template: any) => {
        ...
        const headerMappings = [...new Set(headerMatches)].map(m => ({
        ...
        const bodyMappings = [...new Set(bodyMatches)].map(m => ({
            index: parseInt(m.toString().replace(/{{|}}/g, '')),
            value: '',
            mappingType: 'STATIC' as const,
    ...
    const handleLaunch = async () => {
        ...
        try {
            ...
            if (result.success) {
                toast.success(`${result.message}. ID: ${result.messageId}`);
                setSelectedGuests([]);
            } else {
                                    ...
                                    <div key={s.id} className={cn(
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                {s.metadata?.errors?.[0] ? (
                                                    <p className="text-red-600 font-bold text-[9px]">
                                                        Error: {s.metadata.errors[0].message}
                                                    </p>
                                                ) : (
                    ...
                    <div className="p-6 border-b border-slate-100 bg-white">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div className="flex gap-2">
                                ...
                                >
                                    <Hotel className="w-4 h-4" /> Hotel guests
                                </button>
                                <button
                                ...
                                >
                                    <Utensils className="w-4 h-4" /> Restaurant users
                                </button>
                            </div>
                        </div>
