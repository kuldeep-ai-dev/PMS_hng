...
import { Clock, CheckCircle2, ChefHat, AlertCircle, Volume2, VolumeX, Ghost, Receipt, Ban, Info, User, Hash, Printer, Split, Wallet } from 'lucide-react';
...
export default function RestaurantOrders() {
  ...
  const fetchLoyaltySettings = async () => {
    const { data } = await supabase.from('restaurant_loyalty_settings').select('*').limit(1).maybeSingle();
    if (data) setLoyaltySettings(data);
  };

  const fetchHotelSettings = async () => {
    const { data } = await supabase.from('hotel_settings').select('*').limit(1).maybeSingle();
  ...
  }, [mounted, isAudioEnabled]);

  useEffect(() => {
    if (mounted) localStorage.setItem('geny_pms_order_alerts', isAudioEnabled.toString());
  }, [isAudioEnabled, mounted]);
    ...
    const html = `
      <html>
        ...
        <body onload="window.print(); window.close();">
          <div class="header">
            <div style="font-size: 10px; font-weight: 800; text-transform: uppercase; margin-bottom: 2mm;">${hotelSettings?.hotel_name || 'Hotel New Ganga'}</div>
