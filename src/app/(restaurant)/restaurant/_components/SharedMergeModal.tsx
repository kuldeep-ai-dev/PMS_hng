...
import { X, CheckCircle2, ArrowRightLeft, Utensils } from 'lucide-react';
import { toast } from 'sonner';
import { cn } from '@/lib/utils';

export function SharedMergeModal({
            ...
            <div className="relative bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="p-8 border-b border-slate-50 flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-slate-500 font-bold text-xs mt-1">Move or Merge Order</p>
                    </div>
                ...
                <div className="p-8 space-y-6">
                    ...
                    {selectedTargetTableId && (
                        <div className="p-6 bg-slate-50 rounded-[28px] border border-slate-100 animate-in fade-in slide-in-from-top-2 duration-300 text-center">
                            <p className="text-[11px] font-bold text-slate-500 leading-relaxed italic">
                                {tables.find((t: any) => t.id === selectedTargetTableId)?.status.toLowerCase() === 'occupied'
                                    ? "This will combine all items from the current order into the target table's bill."
