-- Fix execute_night_audit signature and permissions
...
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER -- Ensures it runs with elevated privileges to update bookings/logs
...
BEGIN
    ...
    FOR v_booking IN 
        SELECT b.id AS booking_id, b.room_id, r.base_rate, b.guest_id, b.pax_count, b.extra_bed, b.meal_plan
        FROM bookings b
    ...
    LOOP
        ...
        IF v_booking.extra_bed THEN
            v_base_rate := v_base_rate + v_extra_bed_rate;
        END IF;
        ...
        -- Apply Tax (12% fixed for now or as per system standard)
        v_charge_amount := v_base_rate * 1.12; 
...
GRANT EXECUTE ON FUNCTION execute_night_audit(DATE, UUID, JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION execute_night_audit(DATE, UUID, JSONB) TO service_role;

-- Ensure night_audit_logs is readable by authenticated
GRANT SELECT, INSERT ON TABLE night_audit_logs TO authenticated;
GRANT ALL ON TABLE night_audit_logs TO service_role;
